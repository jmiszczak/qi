...
\parindent=0pt
\begin{document}
\title{QI Package for \emph{Mathematica} 7.0 (version 0.3.2)}\author{Jaros{\l}aw Miszczak \quad Piotr Gawron \quad Zbigniew Pucha{\l}a\\
...
\textbf{$ \text{ApplyKraus} $ }-- ApplyKraus[ck,$\rho $] - apply channel ck, given as a list of Kraus operators, to the input state $\rho $.$  $\\
...
\textbf{$ \text{ChannelQ} $ }-- Performs some checks on Kraus operators. Use this if you want to check if they represent quantum channel.$  $\\
...
\textbf{$ \text{cnot} $ }-- Controlled not matrix for two qubits. This is predefined constant.$  $\\
...
\textbf{$ \text{Concurrence4} $ }-- Concurrence4[$\rho $] returns quantum concurrence of the density matrix $\rho $ representing a state of two-qubit system.$  $\\
...
\textbf{$ \text{Fidelity} $ }-- Fidelity[$ \rho _1,\rho _2 $] returns the quantum fidelity between states $ \rho _1 $ and $ \rho _2 $ calculated using simplifide formula as ($\sum $$ \lambda _i)^2 $, where $ \lambda _i $ are the eigenvalues of $ \rho _1\rho _2. $\\
...
\textbf{$ \text{id} $ }-- Identity matrix for one qubit. See also: IdentityMatrix. This is predefined constant.$  $\\
...
\textbf{$ \text{NumericalRangeBound} $ }-- NumericalRangeBound[A$\_$,step$\_$:0.01] - bound of numerical range of matrix A calculated with given step. Ref: Carl C. Cowen, Elad Harel, An Effective Algorithm for Computing the Numerical Range. Technical report, Dep. of Math. Purdue University, 1995.$  $\\
...
\textbf{$ \text{qiAbout} $ }-- Display short information about the QI package.$  $\\

\textbf{$ \text{qiConstInfo} $ }-- qiConstInfo$ \text{::} $usage$  $\\

\textbf{$ \text{qiGenDoc} $ }-- Generate documentation for the QI package. This function accepts up to two arguments - file name and output directory. By default the first one is set to 'qi$\_$usage.tex' and the second one to '$\sim $/zksi-repo/qi/doc'.$  $\\

\textbf{$ \text{qiHistory} $ }-- Display the history of modifications for the QI package.$  $\\

\textbf{$ \text{qiLastModification} $ }-- qiLastModification$ \text{::} $usage$  $\\

\textbf{$ \text{qiVersion} $ }-- Display version of the QI package.$  $\\
...
\textbf{$ \text{SchmidtDecomposition} $ }-- SchmidtDecomposition[vec,d1,d2] - Schmidt decomposition of the $\vdots $tor vec in d1$\times $d2-dimensional Hilbert space.$  $\\
...
\textbf{$ \text{sx} $ }-- Pauli matrix $ \sigma _x $. This is predefined constant.$  $\\

\textbf{$ \text{sy} $ }-- Pauli matrix $ \sigma _y $. This is predefined constant.$  $\\
...
\textbf{$ \text{SymbolicVector} $ }-- SymbolicVector[a,n] is equivalent to Matrix[a,n,1] and it returns a vector with m elements a[i],i=1,...,n. This function is useful, for example, for generating lists of parameters.$  $\\

\textbf{$ \text{sz} $ }-- Pauli matrix $ \sigma _z $. This is predefined constant.$  $\\

\textbf{$ \text{TraceDistance} $ }-- TraceDistance[A,B] returns the trace distance between matrices A and B which is defined as $ \frac{1}{2} $tr$|$A-B$|$.$  $\\
...
\textbf{$ \text{wh} $ }-- Hadamard gate for one qubit. See also: QFT. This is predefined constant.$  $\\

\textbf{$ X $ }-- Generalized Pauli matrix X. See also: $ \sigma _x $\\

\textbf{$ Z $ }-- Generalized Pauli matrix Z. See also: $ \sigma _z $\\
...
\textbf{$ \text{$\lambda $1} $ }-- Gell-Mann matrix $ \lambda _1 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $2} $ }-- Gell-Mann matrix $ \lambda _2 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $3} $ }-- Gell-Mann matrix $ \lambda _3 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $4} $ }-- Gell-Mann matrix $ \lambda _4 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $5} $ }-- Gell-Mann matrix $ \lambda _5 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $6} $ }-- Gell-Mann matrix $ \lambda _6 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $7} $ }-- Gell-Mann matrix $ \lambda _7 $. This is predefined constant.$  $\\

\textbf{$ \text{$\lambda $8} $ }-- Gell-Mann matrix $ \lambda _8 $. This is predefined constant.$  $\\

\textbf{$ \text{$\sigma $x} $ }-- Pauli matrix $ \sigma _x $. This is predefined constant.$  $\\

\textbf{$ \text{$\sigma $y} $ }-- Pauli matrix $ \sigma _y $. This is predefined constant.$  $\\

\textbf{$ \text{$\sigma $z} $ }-- Pauli matrix $ \sigma _z $. This is predefined constant.$  $\\
