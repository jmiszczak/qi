...
\parindent=0pt
\begin{document}
\title{QI Package for \emph{Mathematica} 7.0 \\(version 0.3.6)}\author{Jaros{\l}aw Adam Miszczak \quad Piotr Gawron \quad Zbigniew Pucha{\l}a\\
...
\noindent\textbf{$ \text{QubitDaviesDynamicalMatrix} $ }-- Returns dynamical matrix for Davies channel with b = $ \frac{a p}{1-p}. $\\[8pt]
...
\noindent\textbf{$ \text{RandomSimplex} $ }-- RandomSimplex[d] - d-dimensional random simplex.$  $\\[8pt]
