...
\textbf{$ \text{QubitDaviesDynamicalMatrix} $ }-- QubitDaviesDynamicalMatrix[a,c,p] returns a dynamical matrix for one-qubit Davies channel with parameters a and c and stationary state (p,1-p).$  $\\
...
\textbf{$ \text{RandomUnitVectorSchmit} $ }-- RandomUnitVectorSchmit[n,r] returns a unit vector on n-dimensional space with a Schmidt rank r. Note that r has to smaller or equal $\surd $n and n has to be a perfect square.$  $\\
